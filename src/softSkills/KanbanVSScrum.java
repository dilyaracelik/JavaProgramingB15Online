package softSkills;

public class KanbanVSScrum {


  /*  team capacity :
    number of team member * 8 = team capacity
    Velocity : total poin of user stories on sprint backlog.
    Velocity =< capacity
    grooming meeting : give point for each user story
    sprint planning : give a point + prioritize user stories and create sprint backlog.
            sprint 5 : 1 week     2 week
    sprint plannning should be done in somewhere of sprint 4
    in sprint 5 , we can do sprint 6 planning meeting.
    KT SESSION : Knowledge transfer session
=======================================

    in scrum : sprint backlog 12 user stories
 6 testers.
 1 tester -->2
    complicated , simple
*/

}
